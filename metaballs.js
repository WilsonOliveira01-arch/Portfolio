...
const presets = {
  ...
  nocturne: {
    ...
    specularPower: 6,
    fresnelPower: 1.4,
    backgroundColor: new THREE.Color(0x05080f),
    sphereColor: new THREE.Color(0x0a0f1c),
    lightColor: new THREE.Color(0x7fe0ff),
    lightPosition: new THREE.Vector3(0.8, 1.0, 0.7),
    smoothness: 0.35,
    contrast: 1.9,
    fogDensity: 0.08,
    cursorGlowIntensity: 0.7,
    cursorGlowRadius: 1.6,
    cursorGlowColor: new THREE.Color(0x7fe0ff)
  },
  aqua: {
    sphereCount: isMobile ? 3 : 5,
    ambientIntensity: 0.04,
    ...
    specularPower: 10,
    fresnelPower: 1.8,
    backgroundColor: new THREE.Color(0x070b13),
    sphereColor: new THREE.Color(0x0a2230),
    lightColor: new THREE.Color(0x7ff7ff),
    lightPosition: new THREE.Vector3(0.7, 1.0, 0.8),
    smoothness: 0.45,
    contrast: 1.8,
    fogDensity: 0.09,
    cursorGlowIntensity: 0.45,
    cursorGlowRadius: 1.2,
    cursorGlowColor: new THREE.Color(0x86f5ff)
  }
};
  ...
  material = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      float sceneSDF(vec3 pos) {
        ...
        vec3 p = pos;
        p.xy = rotate2d(p.xy, 0.15);
        p.xz = rotate2d(p.xz, -0.12);
        
        float cursorLift = smoothstep(1.0, 0.0, length(p.xy - uCursorSphere.xy));
        p.y += cursorLift * 0.08 * sin(t * 0.8);
        
        vec3 bigSpherePos = vec3(0.15, -0.8, 0.7);
        vec3 rightSpherePos = vec3(1.05, 0.55, 0.55);
        vec3 leftSpherePos = vec3(-1.4, -0.1, 0.35);
        
        float bigSphere = sdSphere(p - bigSpherePos, 0.85);
        float rightSphere = sdSphere(p - rightSpherePos, 0.55);
        float leftSphere = sdSphere(p - leftSpherePos, 0.38);
        
        float ribbon = MAX_DIST;
        for (int i = 0; i < 7; i++) {
          float fi = float(i);
          float curveT = fi * 0.32 + 0.15;
          vec3 center = vec3(
            0.7 + sin(curveT * 1.5) * 0.38,
            -0.2 + cos(curveT * 1.1) * 0.26,
            -0.2 + curveT * 0.65
          );
          
          vec3 rp = p - center;
          rp.xy = rotate2d(rp.xy, curveT * 0.9);
          rp.xz = rotate2d(rp.xz, curveT * 0.7);
          
          float strip = sdRoundBox(rp, vec3(0.18, 0.035, 0.3), 0.025);
          ribbon = smin(ribbon, strip, 0.1);
        }
        
        result = smin(result, bigSphere, 0.2);
        result = smin(result, rightSphere, 0.18);
        result = smin(result, leftSphere, 0.16);
        result = smin(result, ribbon, 0.14);
      ...
      void main() {
        ...
        if (t > 0.0) {
          ...
          vec3 bg = uBackgroundColor.rgb
            + vec3(0.02, 0.05, 0.08) * (1.0 - vignette)
            + vec3(0.0, 0.02, 0.06) * (0.5 + 0.5 * uv.x)
            + vec3(0.0, 0.03, 0.06) * smoothstep(-0.2, 0.8, uv.y);
          
          bg += glowContribution * 0.25;
...
function setupUI() {
  ...
  const metaballFolder = pane.addFolder({ title: "Shapes" });
